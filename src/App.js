...
function generatePromptPayPayload(phone, amount) {
  ...
  const merchant = f("29", f("00","A000000677010111") + f("01", num));
  let p = f("00","01")+f("01","12")+merchant+f("53","764")+f("54",amount.toFixed(2))+f("58","TH")+f("59","Rin")+f("60","Bangkok")+"6304";
  return p + crc16(p);
}
...
function QRCode({ data, size = 220 }) {
  ...
  useEffect(() => {
    setSrc(`https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(data)}&format=svg&margin=10`);
  }, [data, size]);
  return src
    ? <img src={src} width={size} height={size} alt="QR PromptPay" style={{ display:"block" }} />
...
const S = {
  ...
  hTitle: { color:C.accent, fontSize:21, fontWeight:800, margin:0, userSelect:"none" },
  hSub:   { color:"#A08060", fontSize:12, marginTop:2 },
  backBtn:{ background:"none", border:"1px solid #555", color:"#AAA", borderRadius:8, padding:"6px 14px", fontSize:13, cursor:"pointer" },
  wrap:   { width:"100%", maxWidth:480, padding:"24px 20px", display:"flex", flexDirection:"column", gap:16, boxSizing:"border-box" },
  card:   { background:"#FFF", borderRadius:16, padding:20, boxShadow:"0 2px 16px rgba(200,150,62,0.12)", border:`1px solid ${C.accentLight}` },
  cTitle: { fontSize:15, fontWeight:700, color:C.mid, marginBottom:14 },
  label:  { fontSize:13, color:C.mid, marginBottom:6, display:"block", fontWeight:600 },
  input:  { width:"100%", padding:"12px 14px", borderRadius:10, border:`1.5px solid ${C.accentLight}`, fontSize:16, color:C.dark, background:C.light, outline:"none", boxSizing:"border-box", marginBottom:12 },
  btn:    { width:"100%", padding:14, borderRadius:12, border:"none", background:`linear-gradient(135deg,${C.accent},#A07030)`, color:"#FFF", fontSize:16, fontWeight:700, cursor:"pointer", boxShadow:"0 4px 14px rgba(200,150,62,0.4)" },
  tabRow: { display:"flex", gap:8, marginBottom:12 },
  tab:  (on) => ({ flex:1, padding:10, borderRadius:10, cursor:"pointer", textAlign:"center", fontSize:14, border:`2px solid ${on?C.accent:C.accentLight}`, background:on?C.accentLight:"transparent", color:on?C.mid:"#AAA", fontWeight:on?700:400 }),
  ok:   { background:"#E8F5E9", color:"#2E7D32", borderRadius:8, padding:"10px 14px", fontSize:14, fontWeight:600, textAlign:"center", marginTop:8 },
  err:  { background:"#FFEBEE", color:"#C62828", borderRadius:8, padding:"10px 14px", fontSize:14, fontWeight:600, textAlign:"center", marginTop:8 },
  info: { background:"#E3F2FD", color:"#1565C0", borderRadius:8, padding:"10px 14px", fontSize:14, textAlign:"center", marginTop:8 },
  divider:{ height:1, background:C.accentLight, margin:"12px 0" },
  row:  { display:"flex", justifyContent:"space-between", fontSize:14, color:C.mid, marginBottom:6 },
  badge:{ background:C.dark, color:C.accent, borderRadius:10, padding:"6px 18px", fontWeight:700, fontSize:15, display:"inline-block" },
};

export default function App() {
  const [page, setPage]       = useState("tenant");
  const [roomData, setRoomData] = useState([]); // array from supabase
  ...
  // ===== TENANT =====
  if (page === "tenant") {
    const payload = tData ? generatePromptPayPayload(PROMPTPAY, tData.amount) : null;
    return (
      <div style={S.app}>
        <div style={S.header}>
          ...
          {tData && payload && (
            <div style={S.card}>
              <div style={{ textAlign:"center", marginBottom:12 }}>
                ...
                <div style={{ marginTop:8, fontSize:13, color:C.mid }}>{tData.month}</div>
              </div>
              <div style={S.divider}/>
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:14, padding:"8px 0" }}>
                <div style={{ fontSize:13, color:C.mid }}>ยอดที่ต้องชำระ</div>
                <div style={{ fontSize:38, fontWeight:800, color:C.accent }}>฿{Number(tData.amount).toLocaleString()}</div>
                ...
                <div style={{ fontSize:13, color:C.mid, textAlign:"center", lineHeight:1.8 }}>
                  สแกน QR ด้วยแอปธนาคารหรือแอปใดก็ได้ที่รองรับ PromptPay
                </div>
                <div style={{ background:C.accentLight, borderRadius:10, padding:"10px 16px", fontSize:13, color:C.mid, width:"100%", boxSizing:"border-box" }}>
                ...
                <div style={{ background:"#FFF9E6", border:"1px solid #FFE082", borderRadius:10, padding:"10px 16px", fontSize:13, color:"#7B5800", width:"100%", boxSizing:"border-box" }}>
                  ⚠️ กรุณาตรวจสอบชื่อและยอดเงินก่อนชำระทุกครั้ง
                </div>
              </div>
            </div>
          )}
    ...
    return (
      <div style={S.app}>
        ...
        <div style={S.wrap}>
          ...
          <div style={S.card}>
            <div style={S.cTitle}>📋 ยอดที่บันทึกแล้ว ({saved.length} ห้อง)
              {loading && <span style={{ fontSize:12, color:"#999", marginLeft:8 }}>กำลังโหลด...</span>}
            </div>
            {saved.length === 0 && !loading && (
              <div style={{ fontSize:13, color:"#999", textAlign:"center", padding:"16px 0" }}>ยังไม่มีข้อมูล</div>
            )}
                ...
                <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:`1px solid ${C.accentLight}`, paddingBottom:10, marginBottom:10 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <button
                      ...
                      style={{ background:"none", border:"1px solid #FFCDD2", color:"#E53935", borderRadius:6, padding:"3px 8px", fontSize:12, cursor:"pointer" }}>
                      ลบ
                    </button>
                  </div>
                </div>
